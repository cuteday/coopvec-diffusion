...
std::unique_ptr<nvinfer1::INetworkDefinition> createNetworkFromOnnx(nvinfer1::IBuilder& builder, TRTLogger& logger)
{
	// Specify network creation options.
	// Note: TensorRT-RTX only supports strongly typed networks, explicitly specify this to avoid warning.
	nvinfer1::NetworkDefinitionCreationFlags flags = 1U
	...
	if (!network)
	{
		logError("Failed to create network!");
		return nullptr;
	}
	...
	{
		logError("Invalid ONNX input dimension, expected [1, " + std::to_string(kInputSize) + "]!");
		return nullptr;
	}
	auto output = network->getOutput(0);
	if (!output || output->getDimensions().d[0] != 1 || output->getDimensions().d[1] != kOutputSize)
	{
		logError("Invalid ONNX output dimension, expected [1, " + std::to_string(kOutputSize) + "]!");
		return nullptr;
	}
...
std::unique_ptr<nvinfer1::IHostMemory> createSerializedEngine(TRTLogger& logger)
{
	// Create a builder object.
	auto builder = std::unique_ptr<nvinfer1::IBuilder>(nvinfer1::createInferBuilder(logger));
	if (!builder)
	{
		logError("Failed to create builder!");
		return nullptr;
	}
	...
	if (!builderConfig)
	{
		logError("Failed to create builder config!");
		return nullptr;
	}
	...
	if (!network)
	{
		logError("Failed to create network definition!");
		return nullptr;
	}
...
int main(int argc, char **argv) {
	...
	if (argc > 1 && std::string(argv[1]).find("--onnx=") != std::string::npos)
	{
		...
		logInfo("Enabled parsing for ONNX model: " + kOnnxModelPath);
	}
	...
	if (!serializedEngine)
	{
		logError("Failed to build serialized engine!");
		return EXIT_FAILURE;
	}
	logSuccess("Successfully built the network. Engine size: " + std::to_string(serializedEngine->size()) + " bytes.");

	auto runtime = std::unique_ptr<nvinfer1::IRuntime>(nvinfer1::createInferRuntime(logger));
	if (!runtime)
	{
		logError("Failed to create runtime!");
		return EXIT_FAILURE;
	}
	...
	if (!inferenceEngine)
	{
		logError("Failed to create inference engine!");
		return EXIT_FAILURE;
	}
	...
	if (!runtimeConfig)
	{
		logError("Failed to create runtime config!");
		return EXIT_FAILURE;
	}
	...
	for (int32_t i = 0; i < 5; i++)
	{
		...
		if (!status)
		{
			logError("Failed to execute inference!");
	...
	logSuccess("Successfully ran the network.");
