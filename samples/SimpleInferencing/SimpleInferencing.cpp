...
public:
    ...
    bool Init()
    {
        ...
        HostNetwork net(m_networkUtils);
        if (!net.InitialiseFromFile(GetLocalPath("assets/data").string() + std::string("/disney.ns.bin")))
        {
            Log(Error, "Loaded Neural Shading Network from file failed.");
            return false;
        }
        ...
        NetworkLayout deviceNetworkLayout = m_networkUtils->GetNewMatrixLayout(net.GetNetworkLayout(), MatrixLayout::InferencingOptimal);
