...
namespace {
	...
	inline std::string getLogLevelString(Level level) {
		switch (level)
		{
		case Level::Fatal:
			return "[Fatal]";
	...
	inline std::string getLevelAnsiColor(Level level) {
		switch (level) {
		case Level::Success:
...
void Logger::log(Level level, const std::string& msg, bool terminate) {
